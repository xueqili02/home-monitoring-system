...
def detected_filter(detected, max_count):
    if detected is None:
        return None
    # 根据 bbox_score 进行排序
    sorted_indices = (-detected[:, 4]).argsort()
    sorted_detected = detected[sorted_indices]
    # 选择前 max_count 个边界框
    return sorted_detected[:max_count]
...
if __name__ == '__main__':
    ...
    par.add_argument('--save_out', type=str, default='./output/output.mp4', help='Save display to video file.')
    ...
    while cam.grabbed():
        ...
        if detected is not None:
            ...
            # 选择前 max_count 个边界框
            max_count = 5
            detected = detected_filter(detected, max_count)
        ...
        frame = cv2.putText(frame, 'FPS: %d' % (int(1.0 / (time.time() - fps_time) + 0.5)), (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
        frame = frame[:, :, ::-1]
        fps_time = time.time()
        ...
        if outvid:
            writer.write(frame)
        total += end - start
        total0 += time.time() - start0
        cv2.imshow('frame', frame)
        if cv2.waitKey(33) == 27:
            break
    print(f"Average Time Cost of Each Frame = {total0 / times}")
    print(f"Average Time Cost of Detected Part = {total / times}")
    print(f"Percentage : {total / total0}")
    # Clear resource.
    cam.stop()
    if outvid:
        writer.release()
